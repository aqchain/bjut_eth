...
import (
	...
	"bjut_eth/common"
	"bjut_eth/common/hexutil"
	"bjut_eth/common/math"
	"bjut_eth/core/rawdb"
	"bjut_eth/core/state"
	"bjut_eth/core/types"
	"bjut_eth/ethdb"
...
func SetupGenesisBlock(db ethdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	...
	// 当数据库中没有创世区块
	// 再判断是否有自定义的创世区块信息
	//  0 如果没有自定义创世区块 则使用默认的主网配置
	//  1 如果有自定义创世区块 则将自定义保存到数据库 并返回该创世区块信息
	...
	// 当数据库中已存在创世区块 但是自定义区块配置存在时 检查两者hash是否一致 不一致返回错误
	if genesis != nil {
	...
	// hash检查完成后 检查数据库中是否存在链配置信息 如果配置信息不存在则写入配置信息
...
func DefaultGenesisBlock() *Genesis {
	...
	mainnetAlloc[common.HexToAddress("0x45845649E584C893D581bba91ECC4575429Ac1B1")] = GenesisAccount{Balance: balance}
